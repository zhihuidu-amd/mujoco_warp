...
def _solve(m: types.Model, d: types.Data, ctx: SolverContext):
  ...
  if m.opt.iterations != 0 and m.opt.graph_conditional:
    ...
    # Use warp's own capture detection (torch.cuda.is_current_stream_capturing 
    # fails with hipErrorStreamCaptureImplicit when called during HIP capture)
...
def _solve_islands(m: types.Model, d: types.Data, ctx: IslandSolverContext):
  ...
  if m.opt.iterations != 0 and m.opt.graph_conditional:
    ...
    for i in range(m.opt.iterations):
      ...
      _in_cap_is = _dev.is_capturing if _dev.is_hip else False
